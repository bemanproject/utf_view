...
### Before
```cpp
vector<uint32_t> utf16be_to_utf32be(vector<uint32_t> const utf16be_data) {
  return utf16be_data 
    | views::transform(
        [](const uint16_t x) {
...
struct @*byteswap-functor*@ { // @*exposition only*@/
  constexpr T operator()(auto x) const noexcept { return std::byteswap(x); }
};
```

The name `byteswap` denotes a range adaptor object ([range.adaptor.object]). Let `E` be an expression and let `T` be `remove_cvref_t<decltype((E))>`. If `ranges::range_reference_t<T>` does not model `integral`, `byteswap(E)` is ill-formed. The expression `byteswap(E)` is expression-equivalent to `ranges::transform_view(std::views::all(E), @*byteswap-functor*@{})`.
