...
by the view is the same for every valid choice. Choosing `n > 1` permits an
...
code units at or after `current_`. An implementation locates the position at
which they begin by scanning backward, classifying input subsequence
boundaries from a bounded number of neighboring code units. As with
`@*read*@`, the choice of `n` is not observable; in particular, an
implementation may always choose `n == 1` for `@*read-reverse*@`, since
backward iteration is typically used for short, local movements that would
not amortize the cost of transcoding a large chunk.
...
must locate an earlier boundary first. This is always possible by bounded
backward scanning, because the division into input subsequences under
