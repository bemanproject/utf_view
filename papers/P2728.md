...
(for example, using SIMD instructions) and serve subsequent increments out of
the buffer. If the underlying range is single-pass, the buffer contains the
code units of exactly one character (between one and four code units), because
reading ahead in a single-pass range is destructive and therefore observable.
...
For an implementation whose `@*read*@` transcodes a single input
subsequence per invocation, the first comparison is also equivalent to
...
starting from different positions (for example, when one of them was moved
backward with `operator--`, which refills the buffer with a single input
subsequence).
...
the number of code units read while decoding `c`; replaces the contents of
`buf_` with the code units of `c`, encoded in the UTF encoding corresponding
to `ToType`; and sets `buf_index_` to `buf_.size() - 1`, or to `0` if this is
an `or_error` view and we read an invalid subsequence.

::: note

Unlike `@*read*@`, `@*read-reverse*@` always transcodes a single
input subsequence: when moving backward, the position at which a chunk of
`n > 1` input subsequences would begin is not yet known.
...
- Loosen the wording to admit chunked (e.g. SIMD) implementations when the
  ...
  unspecified constant, allow `@*read*@` to transcode several input
  subsequences per invocation, and respecify `base()` and iterator equality
  positionally instead of in terms of the exposition-only members. Add
  corresponding design discussion.
