...
an exposition-only transformation functor that performs the needed cast. The
rule governing which source types may be cast is discussed in
[Code Unit Adaptor Conversion Rule](#code-unit-adaptor-conversion-rule).
...
```c++
  ...
  enum class to_utf_view_kind : @*unspecified*@;
  ...
  template<input_range V, to_utf_view_kind E, @*code-unit*@ ToType>
    requires view<V> && @*code-unit*@<range_value_t<V>>
  class to_utf_view;

  template<class V, to_utf_view_kind E, class ToType>
...
`to_utf_view_kind` constant template parameter is `expected`, it produces a view of
...
corresponding `to_utf_view_kind`. Let `E` be an expression and let `T`
...
- If `E` is a specialization of `empty_view` ([range.empty.view]):
  ...
  - If `Error` is `to_utf_view_kind::replacement`, then
    `empty_view<Char>{}`.
...
#### 25.7.?.3 Enumeration `to_utf_view_kind` [range.transcoding.error.kind] {-}
...
public:
  ...
  template <auto E2>
    constexpr explicit to_utf_view(V base, constant_wrapper<E2, to_utf_view_kind>, to_utf_tag_t<ToType>)
      requires (constant_wrapper<E2, to_utf_view_kind>::value == E);
  ...
  constexpr auto size() const
    requires sized_range<const V> && same_as<char32_t, range_value_t<const V>> && same_as<char32_t, ToType>
  {
    return size(@*base_*@);
  }
...
template<class R, auto E2, @*code-unit*@ ToType>
  to_utf_view(R&&, constant_wrapper<E2, to_utf_view_kind>, to_utf_tag_t<ToType>)
    -> to_utf_view<views::all_t<R>, constant_wrapper<E2, to_utf_view_kind>::value, ToType>;
...
public:
  ...
  using value_type = conditional_t<E == to_utf_view_kind::expected, expected<ToType, utf_transcoding_error>, ToType>;
...
private:
  ...
  template<input_range V2, to_utf_view_kind E2, @*code-unit*@ ToType2>
...
public:
  ...
  constexpr auto base_code_units() const requires forward_range<@*Base*@>
  {
    return subrange(@*current_*@, next(@*current_*@, @*to_increment_*@));
  }

  constexpr value_type operator*() const;

  constexpr @*iterator*@& operator++() requires (E == to_utf_view_kind::expected)
  {
    if (!@*success*@()) {
  ...
  constexpr @*iterator*@& operator++() requires (E == to_utf_view_kind::replacement)
  {
    @*advance-one*@();
    return *this;
  }
  ...
  friend constexpr auto operator<=>(@*iterator*@ const& lhs, @*iterator*@ const& rhs)
    requires random_access_range<@*Base*@> && same_as<char32_t, range_value_t<@*Base*@>> && same_as<char32_t, ToType>
  {
    return lhs.@*current_*@ <=> rhs.@*current_*@;
  }

  friend constexpr @*iterator*@& operator+=(@*iterator*@& it, difference_type const n)
    requires random_access_range<@*Base*@> && same_as<char32_t, range_value_t<@*Base*@>> && same_as<char32_t, ToType>
  {
    it.@*current_*@ += n;
    it.@*reseat*@();
  ...
  friend constexpr @*iterator*@ operator+(difference_type const n, @*iterator*@ it)
    requires random_access_range<@*Base*@> && same_as<char32_t, range_value_t<@*Base*@>> && same_as<char32_t, ToType>
  {
    it += n;
    return it;
...
private:
  ...
  constexpr void @*reseat*@() { // @*exposition only*@
    if (@*current_*@ != @*end*@())
      @*read*@();
    else {
      @*to_increment_*@ = 0;
  ...
  constexpr expected<void, utf_transcoding_error> @*success*@() const noexcept requires(E == to_utf_view_kind::expected); // @*exposition only*@
...
- If `V` models `random_access_range`, and `range_value_t<V>` is `char32_t`,
  and `ToType` is `char32_t`, then `iterator_concept` is
  `random_access_iterator_tag`.
- Otherwise, if `V` models `bidirectional_range`, then `iterator_concept` is `bidirectional_iterator_tag`.
...
- If `C` models `derived_from<random_access_iterator_tag>`, and
  `range_value_t<V>` is `char32_t`, and `ToType` is `char32_t`, then
...
_Returns_: Either `@*buf_*@[@*buf_index_*@]`, or, if `E` is `to_utf_view_kind::expected` and `!@*success*@()`, then `unexpected{@*success*@().error()}`
...
```cpp
template<class T>
struct @*cast-to*@ { // @*exposition only*@
  constexpr T operator()(auto x) const noexcept { return static_cast<T>(x); }
};
```
...
type for `as_charT`, let `E` be an expression, let `T` be
`remove_cvref_t<decltype((E))>`, and let `U` be
`remove_cvref_t<ranges::range_reference_t<T>>`. `as_charT(E)` is ill-formed if
`T` is an array, or if `U` is not `Char` and `U` is not a type `X` such that
`sizeof(X) == sizeof(Char)` and either `X` is `byte` or `X` is an integral type
other than `bool`. Otherwise, the expression `as_charT(E)` is
expression-equivalent to:

- If `T` is a specialization of `empty_view` ([range.empty.view]), then
  `empty_view<Char>{}`.

- Otherwise, `ranges::transform_view(std::views::all(E), @*cast-to*@<Char>{})`.
...
```cpp
#define __cpp_lib_unicode_transcoding 20XXXXL // freestanding, also in <ranges>
```
...
When we invoke `begin()`, constructing the transcoding iterator may read up to four
elements from the underlying view if it's transcoding from UTF-8. A previous revision of
...
Based on feedback that this was a desired feature, and implementation
experience in similar APIs provided by ICU, this paper introduces a new member
function for the transcoding iterator: `.base_code_units()`. No existing
standard library view provides an equivalent API, so this is novel. It provides
a range of iterators from the underlying range delimiting the code units that
make up the current code point.

This member function is only provided for forward ranges. Previously, there had
been discussion about providing this member function for input ranges as
well. However, there were no good options for making this work. We could give
out iterators to a special cache that's stored in the iterator, but then the
result of `.base_code_units()` would be invalidated after incrementing the
iterator. We could also change the return type of `.base_code_units()` to
something like `std::inplace_vector<char8_t, 4>`, but then separate invocations
of `it.base_code_units().begin()` and `it.base_code_units().end()` on the same
iterator would point to different objects.

Based on these concerns, SG9 voted against providing `.base_code_units()` for
input ranges in Brno:
...
But we voted in favor of providing it for forward ranges:
...
## Code Unit Adaptor Conversion Rule

For the `as_charT` casting views, the rule we use to determine whether the element type
`U` of a view's source range makes it eligible for conversion to the target type `Char`
is:

- `U` is `Char`, or
- `U` is `std::byte` or a non-`bool` integral type, and `sizeof(U) == sizeof(Char)`.

This accomplishes the following design goals:

- Rules out casts which are either illogical (floats, bools) or which are likely to be
  mistakes (class types, enums other than `std::byte`)
- Rules out narrowing and widening conversions by the width requirement, including between
  `charN_t` types
- Allows conversions from same-width integral types and `std::byte` to `charN_t` types,
  such that `int` can be used to store `char32_t` (as is done in ICU) and `unsigned short`
  can be used for `char16_t` (assuming those types are appropriately sized), and `wchar_t`
  can either be cast to `char16_t` or `char32_t` depending on its width.

This is consistent with [@P3695R3]'s deprecation of most implicit conversions between
`charN_t` types. Despite the fact that that paper only drops implicit casts, and the
`as_charT` views can be likened to an explicit cast, I think it's appropriate here to only
bless casts that we think usually make sense, and make the user work a bit harder to spell
out a `transform_view` if, for example, they want to cast between `char16_t` and
`char32_t` (whose implicit conversions [@P3695R3] preserved, but which I don't think
merits an explicit carveout here since there's no justification based on backwards
compatibility).

| Source | `as_char` | `as_char8_t` | `as_char16_t` | `as_char32_t` | `as_wchar_t` |
|---|---|---|---|---|---|
| `char` | yes | yes | no | no | no |
| `signed char`, `unsigned char` | yes | yes | no | no | no |
| `std::byte` | yes | yes | no | no | no |
| `char8_t` | yes | yes | no | no | no |
| `char16_t` | no | no | yes | no | same width |
| `char32_t` | no | no | no | yes | same width |
| `wchar_t` | no | no | same width | same width | yes |
| `short`, `unsigned short` | no | no | yes | no | same width |
| `int`, `unsigned int` | no | no | no | same width | same width |
| `long`, `unsigned long` | no | no | no | same width | same width |
| `long long`, `unsigned long long` | no | no | no | no | no |
| `bool` | no | no | no | no | no |
| floating-point types | no | no | no | no | no |
| enumerations other than `std::byte` | no | no | no | no | no |
| class types | no | no | no | no | no |
...
- Rename `to_utf_view_error_kind` to `to_utf_view_kind` per SG9 discussion
- Implement `it.base_code_units()` for forward ranges per SG9 feedback
- Add random access support for UTF-32-to-UTF-32 self-transcode per SG9
  feedback
- Constrain the code unit adaptors to same-width integral and `std::byte`
  sources per SG16 feedback, and add a design discussion of the rule
...
## SG16 review of P2728R14 on 2026-06-24 (Telecon)

- [Minutes](https://wiki.isocpp.org/2026_Telecons:SG16Teleconference2026-06-24)
