...
## Why the view can't support SIMD for input views

Unfortunately, although SIMD UTF transcoding is faster, it doesn't fit into the shape of
the C++ views model.

Say a user writes the following function, which reads from an input range until it sees
invalid input, and cuts off the result at that point:
...
In the current implementation, this stops reading from the input range immediately upon
encountering invalid input.

In a SIMD-backed implementation, on the other hand, the transcoding view needs to fill up
the entire SIMD register, and so, for example, it may consumed an additional 15 bytes from
the input view even if the first byte it saw was invalid. Not only does this cause us to
expend resources on a presumptively misbehaving source of input, but it also leaves the
input in an unexpected state, having dropped part of the range past the end of the invalid
UTF-8 bytes on the floor.
