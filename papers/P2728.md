...
We would be introducing, for, to my understanding, the first time, a view that reads its
input in chunks for reasons of performance rather than correctness.
...
unit. This means that, whereas the scalar implementation of `.base()` is a simple accessor
...
The SIMD-based implementation, although it may not match the gold-standard `simdutf`'s
bulk-optimized transcoding performance, is significantly faster than the scalar
implementation. Preliminary performance numbers for the UTF-16 to UTF-8 direction are
below:

// TODO Claude

At the time of publishing R14, SIMD support in the reference implementation is
unfortunately in an incomplete state.
