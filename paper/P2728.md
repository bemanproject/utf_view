...
The `@*utf-iterator*@` stores an iterator pointing to the start of the
character it's transcoding, and a back-pointer to the underlying range in
order to bounds check its beginning and end (which is required for
...
fourth character. `base` points to the start of the fourth character in the
...
`@*read_reverse*@()`. The `@*read()*@` and `@*read_reverse*@()` functions
contain most of the actual transcoding logic, updating `base` and filling
`buf` up with the transcoded characters.
...
namespace std::ranges {
  ...
  public:
    ...
    private:
      ...
      using @*backptr-type*@ = conditional_t<copyable<ranges::iterator_t<V>>, @*to-utf-view-impl*@ const*,
                                             @*to-utf-view-impl*@*>; // @*exposition only*@
    ...
    private:
      constexpr @*utf-iterator*@(@*backptr-type*@ parent, @*iter*@ begin) // @*exposition only*@
    ...
    public:
      ...
      constexpr @*utf-iterator*@& operator=(@*utf-iterator*@ const&) requires copyable<@*iter*@> = default;
      ...
      constexpr @*utf-iterator*@& operator--() requires bidirectional_iterator<@*iter*@>
      {
        if (!buf_index_)
          read_reverse();
        else
      ...
      constexpr @*utf-iterator*@ operator--(int) requires bidirectional_iterator<@*iter*@>
      {
        auto retval = *this;
        --*this;
        return retval;
      ...
      friend constexpr bool operator==(@*utf-iterator*@ const& lhs, @*sent*@ rhs) requires copyable<@*iter*@>
      {
        if constexpr (forward_iterator<@*iter*@>) {
          return lhs.base() == rhs;
        } else {
  ...
  private:
    ...
    constexpr auto make_begin(this auto& self) { // @*exposition only*@
      constexpr bool const_{is_const_v<remove_reference_t<decltype(self)>>};
      return @*utf-iterator*@<const_>(&self, ranges::begin(self.base_));
    }
    constexpr auto make_end(this auto& self) { // @*exposition only*@
      if constexpr (bidirectional_iterator<ranges::sentinel_t<V>>) {
        constexpr bool const_{is_const_v<remove_reference_t<decltype(self)>>};
        return @*utf-iterator*@<const_>(&self);
      } else {
        return ranges::end(self.base_);
      }
    }
  ...
  template<@*from-utf-view*@ V>
  class to_utf16_or_error_view : public ranges::view_interface<to_utf16_or_error_view<V>> {
  private:
  ...
  template<@*from-utf-view*@ V>
  class to_utf32_or_error_view : public ranges::view_interface<to_utf32_or_error_view<V>> {
  private:
...
implementation details common to the six aforementioned transcoding views.
...
which is that that `char8_t` corresponds to UTF-8, `char16_t`
corresponds to UTF-16, and `char32_t` corresponds to UTF-32.
...
being adapted, then `base()` == `@*end()*@`; otherwise, the position of
`base()` is always at the end of the input subsequence corresponding to the
current code point `c`, and `buf_` contains the code units that comprise `c`,
in the UTF encoding corresponding to `ToType`.

For forward and bidirectional iterators, the invariant is: if `*this` is at
the end of the range being adapted, then `base()` == `@*end()*@`; otherwise,
the position of `base()` is always at the beginning of the input subsequence
corresponding to the current code point `c` within the underlying range, and
`buf_` contains the code units in `ToFormat` that comprise `c`.

The exposition-only member function `read` decodes the input subsequence
starting at position `base()` into a code point `c`, using the UTF encoding
...
`c` into `buf_` in the UTF encoding corresponding to `ToType`; and sets
`buf_index_` to `0`. If `forward_iterator<I>` is `true`, `base()` is set to
the position it had before `read` was called.
...
`to_increment_` to the number of code units read while decoding `c`; encodes
`c` into `buf_` in the UTF encoding corresponding to `ToType`; and sets
`buf_index_` to `buf_.size() - 1`, or to `0` if this is an `or_error` view and
...
the beginning of its underlying range if `buf_index_` is zero and `base() ==
begin()`. If it is a `forward_iterator`, it is defined to be at the end of its
underlying range if `buf_index_ + 1 == buf_.size()` and `base() ==
@*end()*@`. Otherwise, it is defined to be at the end of its underlying range
if `buf_index_ == buf_.size()` and `base() == @*end()*@`.
