...
public:
  ...
  constexpr @*iterator*@<true> begin() const requires range<const V> &&
           same_as<range_value_t<V>, char32_t>;
...
public:
  ...
  constexpr @*iterator*@(@*Parent*@* parent, iterator_t<@*Base*@> begin) : current_(std::move(begin)), parent_(parent) {
    if (base() != @*end*@())
      read();
  }
...
`@*to-utf-view-impl*@::@*iterator*@` is an iterator that transcodes from UTF-N
to UTF-M, where N and M are each one of 8, 16, or 32. N may equal
M. `@*to-utf-view-impl*@::@*iterator*@` uses a mapping between character types
and UTF encodings, which is that that `char8_t` corresponds to UTF-8,
`char16_t` corresponds to UTF-16, and `char32_t` corresponds to UTF-32.
...
encoding, into an internal code unit buffer.

`@*to-utf-view-impl*@::@*iterator*@` maintains certain invariants; the
invariants differ based on whether `@*to-utf-view-impl*@::@*iterator*@` is an
input iterator.
...
If `@*to-utf-view-impl*@::@*iterator*@` is a `bidirectional_iterator`, it is
defined to be at the beginning of its underlying range if `buf_index_` is zero
and `base() == begin()`. If it is a `forward_iterator`, it is defined to be at
the end of its underlying range if `buf_index_ + 1 == buf_.size()` and `base()
== @*end*@()`. Otherwise, it is defined to be at the end of its underlying
range if `buf_index_ == buf_.size()` and `base() == @*end*@()`.

If `operator*` or `operator++` are invoked while
`@*to-utf-view-impl*@::@*iterator*@` is at the end of its underlying range,
the behavior is undefined; if `operator--` is invoked while
`@*to-utf-view-impl*@::@*iterator*@` is at the beginning of its underlying
