...
_Remarks:_ In order to provide the amortized constant time complexity required by the `range` concept when `to_utf_view` transcodes from UTF-8 or UTF-16, this function caches the result within the `to_utf_view` for use on subsequent calls.
